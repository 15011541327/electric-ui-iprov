<template>
  <div>
    ...
    <!-- 列表显示 -->
    <el-table :data="tableData" ref="aisleTable" border stripe v-loading="loading" style="width: 100%" :height="tableHeight" show-overflow-tooltip @selection-change="handleCurrentChange" @row-dblclick="detailClick">
      <el-table-column type="selection"  fixed="left" width="40"></el-table-column>
      <el-table-column type="index" fixed="left" label="序号" align="center" width="56"></el-table-column>
      <el-table-column prop="memberCode" show-overflow-tooltip fixed="left" width="128" align="center" label="售电公司编码"></el-table-column>
      <el-table-column prop="memberName" show-overflow-tooltip fixed="left" width="184" align="left" label="售电公司名称"></el-table-column>
      <el-table-column prop="shortName" show-overflow-tooltip width="184" align="left" label="售电公司简称"></el-table-column>
      ...
      <el-table-column prop="taxRate" show-overflow-tooltip width="184" align="center" label="增值税率"></el-table-column>
      <el-table-column prop="agreementCode" show-overflow-tooltip width="156" align="center" label="使用协议书编号"></el-table-column>
      <el-table-column prop="memberTypeName" show-overflow-tooltip width="184" align="center" label="售电公司类型"></el-table-column>
      <el-table-column prop="superiorUnitName" show-overflow-tooltip width="184" align="center" label="上级单位名称"></el-table-column>
      <el-table-column prop="proxyCapacity" show-overflow-tooltip width="184" align="center" label="代理负荷容量"></el-table-column>
      ...
      <el-table-column prop="buyLevelName" show-overflow-tooltip width="184" align="center" label="购(售)电层级"></el-table-column>
      <el-table-column prop="settleCompanyId" show-overflow-tooltip width="184" align="center" label="购(售)电结算单位"></el-table-column>
...
export default {
  name: 'saleMemberForIndex',
  data: function () {
    return {
      enum_code: 'CLASS_TYPE:classType,MEMBER_STATUS:memberStatus,MEMBER_TYPE_SDGS:memberType,BUY_LEVEL:buyLevel,DATA_SOURCE:dataSource',
      enum_data: [],
  ...
  methods: {
    ...
    // 新增
    insertClick () {
      _this.layerOpen('新增', require('./saleMemberForInsert'), { selectData: _this.enum_data })
    },
    // 修改
    updateClick () {
      if (this.choiceData.length === 0) {
        ...
        _this.layerOpen('修改', require('./saleMemberForUpdate'), data)
      }
    },
    // 双击打开详情
    detailClick (row) {
      ...
      _this.layerOpen('售电公司详情', require('./saleMemberForDetail'), data)
    },
    // 打开弹窗
    ...
    exportClick () {
      if (_this.choiceData.length > 0) {
        let tableHeader = ['售电公司编码', '售电公司名称', '售电公司简称', '市场成员类型', '统一社会信用代码', '法人代表', '注册地址', '注册资金',
          '开户银行', '开户名称', '银行账号', '银行联行号', '增值税率', '使用协议书编号', '售电公司类型', '上级单位名称', '代理负荷容量',
          '地理区域', '控制区域', '购(售)电层级', '购(售)电结算单位', '生效时间', '失效时间', '对应供应商编码',
          '市场成员状态', '数据来源'
        ]
        let tableVal = ['memberCode', 'memberName', 'shortName', 'classTypeName', 'socialCode', 'legalRepre', 'address', 'reditMoney', 'acctBankName',
          'acctName', 'acctCode', 'acctLiaison', 'taxRate', 'agreementCode', 'memberTypeName', 'superiorUnitName', 'proxyCapacity',
          'geogRegion', 'controlRegion', 'buyLevelName', 'settleCompanyId', 'startTime', 'endTime', 'supplierCode',
          'memberStatusName', 'dataSourceName'
        ]
        _this.$curdUtil.exportChoiceData(_this, tableHeader, tableVal, '售电公司')
      } else {
        _this.$curdUtil.export(_this, _this.queryData, '售电公司.xls', url.export)
      }
    }
  }
}
