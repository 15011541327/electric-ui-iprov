<template>
  <div>
    ...
    <el-table :data="tableData" ref="aisleTable" border stripe v-loading="loading" style="width: 100%" :height="tableHeight" show-overflow-tooltip @selection-change="handleCurrentChange" @row-dblclick="detailClick">
      ...
      <el-table-column prop="memberCode" show-overflow-tooltip fixed="left" width="128" align="center" label="电力用户编码"></el-table-column>
      <el-table-column prop="memberName" show-overflow-tooltip fixed="left" width="184" align="left" label="电力用户名称"></el-table-column>
      <el-table-column prop="shortName" show-overflow-tooltip width="184" align="left" label="电力用户简称"></el-table-column>
      ...
      <el-table-column prop="memberTypeName" show-overflow-tooltip width="184" align="center" label="用户类型"></el-table-column>
      <el-table-column prop="superiorUnitName" show-overflow-tooltip width="184" align="center" label="上级单位名称"></el-table-column>
      <el-table-column prop="voltageGradeName" show-overflow-tooltip width="184" align="center" label="用户供电电压等级"></el-table-column>
      <el-table-column prop="newspaperCapacity" show-overflow-tooltip width="184" align="center" label="报装容量"></el-table-column>
      <el-table-column prop="zdxl" show-overflow-tooltip width="184" align="center" label="最大需量"></el-table-column>
...
export default {
  name: 'powerUserForIndex',
  data: function () {
    return {
      enum_code: 'MEMBER_TYPE_YHLX:memberType,CLASS_TYPE:classType,MEMBER_STATUS:memberStatus,VOLTAGE_GRADE:voltageGrade,BUY_LEVEL:buyLevel,DATA_SOURCE:dataSource',
      enum_data: [],
  ...
  methods: {
    ...
    // 新增
    insertClick () {
      _this.layerOpen('新增', require('./powerUserForInsert'), { selectData: _this.enum_data })
    },
    // 修改
    updateClick () {
      if (this.choiceData.length === 0) {
        ...
        _this.layerOpen('修改', require('./powerUserForUpdate'), data)
      }
    },
    // 双击打开详情
    detailClick (row) {
      ...
      _this.layerOpen('电力用户详情', require('./powerUserForDetail'), data)
    },
    // 打开弹窗
    ...
    exportClick () {
      if (_this.choiceData.length > 0) {
        let tableHeader = ['电力用户编码', '电力用户名称', '电力用户简称', '市场成员类型', '统一社会信用代码', '法人代表', '注册地址', '注册资金',
          '开户银行', '开户名称', '银行账号', '银行联行号', '增值税率', '使用协议书编号', '用户类型', '上级单位名称', '用户供电电压等级',
          '报装容量', '最大需量', '地理区域', '控制区域', '生效时间', '失效时间', '对应供应商编码', '市场成员状态', '数据来源'
        ]
        let tableVal = ['memberCode', 'memberName', 'shortName', 'classTypeName', 'socialCode', 'legalRepre', 'address', 'reditMoney', 'acctBankName',
          'acctName', 'acctCode', 'acctLiaison', 'taxRate', 'agreementCode', 'memberTypeName', 'superiorUnitName', 'voltageGradeName',
          'newspaperCapacity', 'zdxl', 'geogRegion', 'controlRegion', 'startTime', 'endTime', 'supplierCode',
          'memberStatusName', 'dataSourceName'
        ]
        _this.$curdUtil.exportChoiceData(_this, tableHeader, tableVal, '电力用户')
      } else {
        _this.$curdUtil.export(_this, _this.queryData, '电力用户.xls', url.export)
      }
    }
  }
}
